...
import static ca.pfv.spmf.test.MainTestRuleGrowth.fileToPath;
import ca.pfv.spmf.tools.MemoryLogger;
import java.io.File;

/**
 * This is the original implementation of the RULEGROWTH algorithm for mining
 * sequential rules common to several sequences where antecedent and consequent
 * are unordered itemsets. The RuleGrowth algorithm is described in this paper:
 * <br/><br/>
 * Fournier-Viger, P., Nkambou, R. & Tseng, V. S. (2011). RuleGrowth: Mining
 * Sequential Rules Common to Several Sequences by Pattern-Growth. Proceedings
 * of the 26th Symposium on Applied Computing (ACM SAC 2011). ACM Press, pp.
 * 954-959.
 * <br/><br/>
...
public class AlgoRULEGROWTH {
    ...
    public static void main(String[] arg) throws IOException {
        String input = "C:\\Users\\Animesh2\\Desktop\\Data\\methodInClassFile.txt";  // the database
        File sequentialRules = new File("C:\\Users\\Animesh2\\Desktop\\Data\\MiningOutput.txt");  // the path for saving the frequent itemsets found
...
//		algo.setMaxConsequentSize(2);  // optional
        algo.runAlgorithm(input, sequentialRules.getAbsolutePath(), minsup_relative, minconf);
...
//		   algo.runAlgorithm(minsup_absolute, minconf, input, output);
        // print statistics
        algo.printStats(sequentialRules);
    }

    public void excuteMain(String input, int minsup_relative, double minconf, File sequentialRules) throws IOException {
//        String input = "C:\\Users\\Animesh2\\Desktop\\Data\\methodInClassFile.txt";  // the database
        //  Applying RuleGROWTH algorithm with minsup = 3 sequences and minconf = 0.5
//        int minsup_relative = 3;
//        double minconf = 0.9;
...
//		   algo.runAlgorithm(minsup_absolute, minconf, input, output);
     ...
     * The main method to run the algorithm
     *
     * @param relativeMinsup : the minimum support as an integer value (a
...
//
     ...
     * with any item c. This results in rules of the form I U�{c} --> J. The
     * method makes sure that: - c is not already included in I or J - c appear
     * at least minsup time in tidsIJ before last occurence of J - c is
     * lexically bigger than all items in I
     *
     * @throws IOException
     */
    ...
    /**
     * This method search for items for expanding left side of a rule I --> J
     * with any item c. This results in rules of the form I --> J U�{c}. The
     * method makes sure that: - c is not already included in I or J - c appear
     * at least minsup time in tidsIJ after the first occurence of I - c is
     * lexically bigger than all items in J
     *
     * @throws IOException
     */
...
//    	return;
     ...
     * @return A map such that key = item value = a map where a key = tid and a
     * value = Occurence This map allows knowing the frequency of each item and
     * their first and last occurence in each sequence.
     */
...
//		System.out.println("NUMBER OF DIFFERENT ITEMS : " + mapItemCount.size());
     ...
     * items in the itemset are sorted in lexical order This version also checks
     * that if the item "item" was added it would be the largest one according
    ...
    /**
     * This method checks if the item "item" is in the itemset. It assumes that
     ...
     */
    public void printStats(File sequentialRules) throws IOException {
        System.out.println("=============  RULEGROWTH - STATS ========");
        System.out.println("Network rules count: " + ruleCount);
        System.out.println("Total time: " + (timeEnd - timeStart) + " ms");
        System.out.println("Max memory: " + MemoryLogger.getInstance().getMaxMemory());
        System.out.println("==========================================");
//        File ruleCountFile = new File(sequentialRules.getParentFile().getParentFile().getParentFile().getAbsoluteFile() + "\\SequentialRuleCount.txt");
        File ruleCountFile = new File(sequentialRules.getParentFile().getParentFile().getParentFile().getAbsoluteFile() + "\\NetworkRuleCount.txt");
        
        BufferedWriter bufwriter = new BufferedWriter(new FileWriter(ruleCountFile, true));
        String versionNumber = sequentialRules.getName(); //.replace("_sequentialRules", "");
        bufwriter.write(versionNumber +"\t" + ruleCount + "\n");//writes the edited string buffer to the new file
